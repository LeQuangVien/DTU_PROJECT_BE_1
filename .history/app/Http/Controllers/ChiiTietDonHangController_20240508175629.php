...
class ChiiTietDonHangController extends Controller
{
    ...
    {
        $khachhang = Auth::guard('sanctum')->user();
        $donhang = ChiiTietDonHang::where('id', $khachhang->id, $request->ds_mua_hang)->get();
        $sanpham = SanPham::where('id', $khachhang->id,'id', $request->id_san_pham)->first();
        $chitietdonhang = ChiiTietDonHang::where('id', $khachhang->id)->first();
        if ($donhang) {
            $donhang = GioHang::create([
                'id_khach_hang' => $khachhang->id,
                'id_san_pham'   => $sanpham->id,
